...
# gs_plot_genesect <- function(gs, type) {
#   ## This function simply iterates through the plot.df and renders the plot with rect and text objects
#   types = c("FET-pval", "FET-odds", "MC-pval", "MC-z")
#   set_list = gs$set_list
#   dim = gs$dim
#   plot.df = gs$plot.df
#   
#   if (type %notin%  types) {
#     stop(str_glue("Error: type ({type}) not in {types}"))
#   }
#   
#   if (type == "FET-pval") {
#     txt=gs$plot.df$text_pval
#     col=gs$plot.df$col_pval
#     lab='Fisher exact test -- p-value'
#     
#   } else if (type == "MC-pval") {
#     txt=gs$plot.df$text_mc
#     col=gs$plot.df$col_mc
#     lab='Montecarlo -- p-value'
#     
#   } else if (type == "MC-z") {
#     txt=gs$plot.df$text_z
#     col=gs$plot.df$col_z
#     lab='Montecarlo -- z-statistic'
#     
#   } else if (type == "FET-odds") {
#     txt=gs$plot.df$text_odds
#     col=gs$plot.df$col_odds
#     lab='Fisher exact test -- odds ratio'
#   }  
#   
#   
#   par(mar=c(1,5,5,1), oma=c(0,0,0,0), xpd=T)#,
#       # mfrow=c(1,2))
#   
#   plot(1,1, type='n',
#        axes=F,
...
#        ylim= c(0,dim))
#   
#   rect(plot.df$x1,plot.df$y1, plot.df$x2, plot.df$y2, col=col)
#   text(plot.df$text.x, plot.df$text.y, txt, 
#        adj=c(0.5,0.5))
#   
#   text(unique(plot.df$text.x), max(plot.df$y2), names(set_list), srt=45, adj=c(-.1,-.1))
#   text(min(plot.df$x1), unique(plot.df$text.y), names(set_list), srt=45, adj=c(1.1,-.1))
#   # text(min(plot.df$x1), min(plot.df$y1), lab, adj=c(0,1.5), cex=1.5, font=2, col='darkred')
#   mtext(lab, side=1, at=0, adj=c(0,0), font=2, col='darkred', cex=1.25)
#   
#   
# }
# 
# gs_plot_ranked_list <- function(gs) {
#   # this function re-orders the plot.df and plots them in the order of a named column
#   rank_dim=70
#   col_width = 4
#   row_width = 1.2
#   
...
#   
#   
#   p.df <- gs$plot.df[order(-gs$plot.df$z),]
#   p.df <- p.df[!is.na(p.df$z), c("x_name", "y_name", "pval", "mc", "z", "odds", "col_pval", "col_mc", "col_z", "col_odds" )]
#   
#   p.df$y = (rank_dim-10) - (0: (nrow(p.df)-1))*row_width
#   # p.df$y = nrow(p.df):1
#   cex=.5
#   
#   p.df$relationship <- "-"
#   p.df$relationship[grepl("_u", p.df$x_name) == grepl("_u", p.df$y_name)] <- "+"
#   
#   par(mar=c(0.1,0.1,0.1,0.1), yaxt='n', xaxt='n')
#   plot(1,1, type='n', xlim=c(0, rank_dim), ylim=c(0,rank_dim), ylab='', xlab='', axes=F)
#   # plotEmpty(rank_dim,rank_dim,T)
#   xs = ((0:5) * col_width) + 10
#   
#   text(xs, rank_dim-8, c("", "", "FET_p", "FET_odds", "MC_p", "z-stat"), cex=cex, font=2, col=c(rep('black',5),"red"))
#   
#   text(xs[1], p.df$y, p.df$x_name, cex=cex)
#   text(xs[2], p.df$y, p.df$y_name, cex=cex)
#   
#   make_rect <- function(x, color) {
#     rect(x-(col_width/2), 
#          p.df$y-(row_width/2), 
#          x+(col_width/2), 
#          p.df$y+(row_width/2), col=color)
#   }
#   make_rect(xs[3], color=p.df$col_pval)
#   make_rect(xs[4], color=p.df$col_odds)
#   make_rect(xs[5], color=p.df$col_mc)
#   make_rect(xs[6], color=p.df$col_z)
#   
#   text(xs[3], p.df$y, p.df$pval, cex=cex)
#   text(xs[4], p.df$y, p.df$odds, cex=cex)
#   text(xs[5], p.df$y, p.df$mc, cex=cex)
#   text(xs[6], p.df$y, p.df$z, cex=cex)
#   
#   text(mean(c(xs[1],xs[2])), p.df$y, p.df$relationship, cex=cex, font=2)
#   
# }
# 
# gs_plot_intersect <- function(gs) {
#   
#   group_offset_factor = 1
#   interbar_gap        = 0.2
#   
#   melted <- reshape2::melt(gs$mats$overlap)
#   
#   pairs = as.data.frame(t(combn(1:length(gs$set_list),2)))
#   names(pairs) <- c("A","B")
#   
#   pairs$nameA <- names(gs$set_list)[pairs$A]
#   pairs$nameB <- names(gs$set_list)[pairs$B]
#   pairs$val <- melted$value[match(paste(pairs$nameA, pairs$nameB),
#                                   paste(melted$Var1, melted$Var2))]
#   padjs = c()
#   oddss = c()
#   
#   for (i in 1:nrow(pairs)) {
#     row = pairs[i,]
#     
#     padj = c(gs$mats$fet_padj[row$nameA, row$nameB], gs$mats$fet_padj[row$nameB, row$nameA])
#     odds = c(gs$mats$fet_odds[row$nameA, row$nameB], gs$mats$fet_odds[row$nameB, row$nameA])
#     padjs = c(padjs, min(padj))
#     oddss = c(oddss, odds[1])
#   }
#   
#   pairs$padj = padjs
#   pairs$odds = oddss
#   pairs$sig_set[pairs$padj < 0.01 & pairs$odds > 1] <- 1
#   pairs$sig_set[pairs$padj < 0.01 & pairs$odds < 1] <- 3
#   pairs$sig_set[is.na(pairs$sig_set)] <- 2
#   
#   pairs$lenA = lengths(gs$set_list[pairs$nameA]) - pairs$val
#   pairs$lenB = lengths(gs$set_list[pairs$nameB]) - pairs$val
#   pairs$tot_width = (pairs$val/2 + max(c(pairs$lenA, pairs$lenB))) * 2
#   
#   
#   
#   pairs = pairs[order(-pairs$sig_set, pairs$val),]
#   
#   xmax = max(pairs$tot_width * 1.2)
#   # ymax = nrow(pairs) + 1
#   xmid = xmax/2
#   
#   
#   pairs$x1 = xmid - pairs$val/2 - pairs$lenA
#   pairs$x2 = xmid - pairs$val/2
#   pairs$x3 = xmid + pairs$val/2
#   pairs$x4 = xmid + pairs$val/2 + pairs$lenB
#   
#   pairs$group_offset = (match(pairs$sig_set, c(3,2,1)) - 1) * group_offset_factor
#   
#   pairs$y1 = 0:(nrow(pairs)-1) + cumsum(rep(interbar_gap, nrow(pairs)))-interbar_gap + pairs$group_offset
#   pairs$y2 = pairs$y1 + 1
#   
#   
#   ymax= max(pairs$y2) * 1.05
#   
#   # graphics.off()
#   par(mar=c(5,.1,2,.1))
#   plot(0,0, type='n', xlim=c(0,xmax), ylim=c(0,ymax), axes=F, ylab='', xlab='Number of Genes', main='Intersection of sets')
#   
#   rect(pairs$x1, pairs$y1, pairs$x2, pairs$y2, col='burlywood1')
#   rect(pairs$x2, pairs$y1, pairs$x3, pairs$y2, col='aliceblue')
#   rect(pairs$x3, pairs$y1, pairs$x4, pairs$y2, col='darkolivegreen1')
#   text(pairs$x1-xmax*.02, pairs$y1+.5, pairs$nameA, adj=c(1,0.5))
#   text(pairs$x4+xmax*.02, pairs$y1+.5, pairs$nameB, adj=c(0,0.5))
#   
#   for (group in c(1,3)) {
#     color = c("red", NA, "dodgerblue")[group]
#     dir   = c("enriched", NA, "depleted")[group]
#     
#     p  = pairs[pairs$sig_set == group,]
#     y1 = min(p$y1) - group_offset_factor/2
#     y2 = max(p$y2) + group_offset_factor/2
#     rect(1, y1, xmax - 1, y2, lty=3, border=color)
#     text(1, y2, str_glue("Sig. {dir}"), adj=c(-.1,-.3),
#          cex=.8, col=color)
#     
#   }
#   
...
# 
# 
# gs_save <- function(gs, base_name=''){
#   graphics.off()
#   svg_dim = length(gs$set_list) 
#   
#   types = c("FET-pval", "FET-odds", "MC-pval", "MC-z")
#   
#   file_name = str_glue("{base_name}FET_pval.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_genesect(gs, types[1])
#   dev.off()
#   fix_svg(file_name)
#   
#   file_name = str_glue("{base_name}FET_odds.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_genesect(gs, types[2])
#   dev.off()
#   fix_svg(file_name)
#   
#   file_name = str_glue("{base_name}MC_pval.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_genesect(gs, types[3])
#   dev.off()
#   fix_svg(file_name)
#   
#   
#   file_name = str_glue("{base_name}MC_zstat.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_genesect(gs, types[4])
#   dev.off()
#   fix_svg(file_name)
#   
#   
#   file_name = str_glue("{base_name}ranked.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_ranked_list(gs)
#   dev.off()
#   fix_svg(file_name)
#   
#   
#   file_name = str_glue("{base_name}intersect.svg")
#   svglite::svglite(file_name, svg_dim, svg_dim)
#   gs_plot_intersect(gs)
#   dev.off()
#   fix_svg(file_name)
# }
# 
# 
# gs_expand_plot <- function(gs, expand, values=F, x.cex=.8, y.cex=0.8) {
#   set_list = gs$set_list
#   
#   x_len = length(set_list[-match(expand, names(set_list))])
#   
#   if (x_len < 1) {
...
#   
#   # expand_comp <- function(x) {
#   #   if (any(x %in% expand)) {
#   #     if (any(x %notin% expand)) {
#   #       return(T)
#   #     }
#   #   }
#   #   return(F)
#   # }
#   # 
#   # p.df <- m.df[apply(m.df[c(1,2)], 1, expand_comp),]
#   
#   x_names = colnames(mat)[-match(expand, colnames(mat))]
#   y_names = expand
#   
...
#   
#   mat_to_val = function(x) {
#     return(gs$mats$fet_odds[x[1], x[2]])
#   }
#   p.df$odds <- apply(p.df[c(1,2)], 1, mat_to_val)
...
#   # }
#   # 
#   # shadify(p.df$odds, pal='Greens 2', log=F)
#   
#   
#   p.df$log_odds <- log2(p.df$odds)
#   get_colors <- function(values, bot_pal, top_pal) {
#     max_range = max(abs(values[!is.infinite(values)]))
#     
#     break_n = 21
#     
#     colors = c( hcl.colors((break_n-1)/2, pal="Reds 3"),
#                 'white', 
#                 rev(hcl.colors((break_n-1)/2, pal="Greens 2")),
#                 "grey60")
#     
#   
#     div = round(((values + max_range) / ( max_range * 2 ) * (break_n-1)) + 1)
#     div[is.infinite(div)] <- 22
#     div
#     return(colors[div])
#   }
#   
...
#   top_pal = "Greens 2"
#   
#   p.df$fill_color <- get_colors(p.df$log_odds, bot_pal = bot_pal, top_pal = top_pal)
#   
#   p.df$text_color <- 'black'
#   p.df$text_color[p.df$pval <= 0.01] <- 'red'
#   p.df$border_lwd <- 1
#   p.df$border_lwd[p.df$pval <= 0.01] <- 1.5
#   
#   
#   
...
#   
#   if (values) {
#     text(p.df$x+.5, p.df$y+.5, round(p.df$log_odds,1), adj=c(0.5,0.5),cex=0.8, col=p.df$text_color)
#   }
#   text(min(p.df$x) - 0.5, p.df$y+0.5, p.df$y_name, adj=c(1,0.5), cex=y.cex)
#   text(p.df$x+0.5, max(p.df$y) + 1.25, p.df$x_name, adj=c(0,0.5), srt=90, cex=x.cex)
#   
#   
#   color_legend <- function(ori.x,ori.y, colors, minmax, dim.x, dim.y) {
#     x2 = ori.x + dim.x
...
#     text(ori.x, ori.y + dim.y/2, minmax[1], adj=c(1.2,0.5))
#     text(ori.x + dim.x, ori.y + dim.y/2, minmax[2], adj=c(-.2,0.5))
#     
#     
#   }
#   break_n = 21
...
#   color_legend(min(p.df$x),0, 
#                colors=c(hcl.colors((break_n-1)/2, pal=bot_pal), 
#                         "white", 
#                         rev(hcl.colors((break_n-1)/2, pal=top_pal))),
#                minmax = c(-minmax, minmax),
#                dim.x=length(unique(p.df$x_name)), dim.y = 0.5)
#   
# }
# 
# # gs_expand_plot(gs, expand, x.cex=.7, y.cex=.7, values=T)
# 
# 
# genesect <- function(set_list, master_set, base_name='', breaks=NULL) {
#   gs <- gs_import(set_list, master_set)
#   gs <- gs_compute_matricies(gs)
#   
#   gs <- gs_make_plot.df(gs, breaks=breaks)
#   gs_save(gs, base_name=base_name)
# 
# }
